...
// ============================================================================
// tb_vector_pipeline_perf.sv
//
// Performance/throughput characterization testbench for the VECTOR pipeline
// in VectorPipelinedCore.
//
// This testbench:
//   1. Exercises a burst of back-to-back VADD instructions.
//   2. Exercises VSTORE + VLOAD through the multi-cycle vector LSU.
//   3. Measures:
//        - total cycles
//        - vector ALU instruction count
//        - ALU pipeline span
//        - measured VALU latency
//        - measured ALU instructions/cycle
//        - measured vector elements/cycle
//        - LSU issue->done latency
//        - LSU busy/stall cycles
//        - vector WB commits
//   4. Prints the architectural parameters used by vector_pkg.
//
// IMPORTANT:
//   This is a PERFORMANCE TESTBENCH. It does not modify your RTL.
//   It uses the existing VectorPipelinedCore hierarchy and only observes
//   vector-pipeline signals.
//
// Expected architectural parameters from vector_pkg:
//   VLEN = 256 bits
//   SEW  = 32 bits
//   LANES = 8
//   MAX_VREG = 32
//
// Test VL = 8, so all 8 physical lanes are active.
// ============================================================================

import vector_pkg::*;
import vector_isa_pkg::*;

module tb_vector_pipeline_perf;
    ...
    // cfg_sel_sew = 0 -> write VL
    // cfg_sel_sew = 1 -> write SEW
    function automatic logic [31:0] vcfginstr(
        input logic       cfg_sel_sew,
        input logic [19:0] data
    );
        vcfginstr = {data, cfg_sel_sew, 4'b0000, VCFG_OP};
    endfunction
    ...
    // ------------------------------------------------------------------------
    // Helper: our ALU benchmark destinations are v4-v15 plus v17.
    // ------------------------------------------------------------------------
    ...
    initial begin
        ...
        //
        // Therefore:
        // v4-v15 = [11,12,13,14,15,16,17,18]
        //
        // v3[31:0] = 0x40 is the LSU base address.
        // -----------------------------
        for (i = 0; i < LANES; i = i + 1) begin
            dut.m_v_regfile.vrf[1][i*SEW +: SEW] = i + 1;
            dut.m_v_regfile.vrf[2][i*SEW +: SEW] = 32'd10;
        end

        dut.m_v_regfile.vrf[3] = '0;
        dut.m_v_regfile.vrf[3][31:0] = 32'h00000040;
        ...
        // 1  : VSETVL VL=8
        //
        // 2-13 : 12 back-to-back VADDs
        //        v4-v15 = v1 + v2
        //
        // 14 : VSTORE v4 -> memory[0x40]
        // 15 : VLOAD  v16 <- memory[0x40]
        // 16 : VADD   v17 = v16 + v2
        //
        // 17+ : scalar NOPs
        //
        // The VADD burst is intentionally independent so that we can
        // measure the pipeline's steady-state VALU throughput.
        // -----------------------------

        dut.m_InstMem.mem[0] = 32'h00000013; // addi x0,x0,0

        dut.m_InstMem.mem[1] =
            vcfginstr(1'b0, 20'd8);          // vsetvl 8
        ...
        dut.m_InstMem.mem[10] =
            vinstr(VADD, 5'd12, 5'd1, 5'd2, UNIT_STRIDE, 4'd0);
        dut.m_InstMem.mem[11] =
            vinstr(VADD, 5'd13, 5'd1, 5'd2, UNIT_STRIDE, 4'd0);
        dut.m_InstMem.mem[12] =
            vinstr(VADD, 5'd14, 5'd1, 5'd2, UNIT_STRIDE, 4'd0);
        dut.m_InstMem.mem[13] =
            vinstr(VADD, 5'd15, 5'd1, 5'd2, UNIT_STRIDE, 4'd0);

        dut.m_InstMem.mem[14] =
            vinstr(VSTORE, 5'd0, 5'd3, 5'd4, UNIT_STRIDE, 4'd0);

        dut.m_InstMem.mem[15] =
            vinstr(VLOAD, 5'd16, 5'd3, 5'd0, UNIT_STRIDE, 4'd0);

        dut.m_InstMem.mem[16] =
            vinstr(VADD, 5'd17, 5'd16, 5'd2, UNIT_STRIDE, 4'd0);

        for (i = 17; i < 64; i = i + 1)
            dut.m_InstMem.mem[i] = 32'h00000013; // scalar NOP
        ...
        // Measurement loop
        // ================================================================
        for (cycle_count = 0; cycle_count < 250; cycle_count = cycle_count + 1) begin

            @(posedge clk);
            #1;

            // -----------------------------
            // Vector instruction in EX
            // -----------------------------
            ...
            // LSU completion
            // -----------------------------
            // vector_lsu.done is level-held until the next LSU issue,
            // so count only its rising edge.
            ...
            // -----------------------------
            // Finish after v17 reaches WB.
            // v17 is the final benchmark instruction.
            // -----------------------------
        ...
        // ================================================================
        // Calculate ALU instruction latency
        // ================================================================
        for (r = 0; r < 32; r = r + 1) begin
            if (is_benchmark_alu_rd(r) &&
                ...
                alu_latency_sum = alu_latency_sum +
        ...
        $display("");
        $display("============================================================");
        $display("       VECTOR PIPELINE PERFORMANCE CHARACTERIZATION");
        $display("============================================================");
        ...
        $display("Test VL              : 8 elements");
        $display("Clock period         : 10 ns");
        $display("Clock frequency      : 100 MHz");

        $display("");
        $display("--- Vector Instruction Activity ---");
        $display("Vector EX instructions : %0d", vector_ex_count);
        $display("Vector ALU EX ops      : %0d", vector_alu_ex_count);
        $display("Vector LSU EX ops      : %0d", vector_lsu_ex_count);
        $display("Vector WB commits      : %0d", vector_wb_count);

        $display("");
        $display("--- VALU Throughput ---");

        if (first_alu_ex_cycle >= 0 && last_alu_ex_cycle >= first_alu_ex_cycle) begin
            $display("ALU benchmark first EX cycle : %0d", first_alu_ex_cycle);
            $display("ALU benchmark last EX cycle  : %0d", last_alu_ex_cycle);
            ...
            $display("Measured elements/cycle         : %0f",
                     ($itor(vector_alu_ex_count) * $itor(VLEN/SEW)) /
                     $itor(last_alu_ex_cycle - first_alu_ex_cycle + 1));
        end
        else begin
        ...
        if (alu_latency_count > 0) begin
            $display("Measured average EX->WB latency : %0f cycles",
                     $itor(alu_latency_sum) / $itor(alu_latency_count));
        end
        else begin
        ...
        $display("--- Overall Run ---");
        if (finished)
            $display("Final benchmark WB cycle : %0d", finish_cycle);
        else
            $display("WARNING: final benchmark instruction did not reach WB");
        ...
        $write("v4 :");
        for (i = 0; i < 8; i = i + 1)
            $write(" %0d", $signed(dut.m_v_regfile.vrf[4][i*SEW +: SEW]));
        $display("");

        $write("v16:");
        for (i = 0; i < 8; i = i + 1)
            $write(" %0d", $signed(dut.m_v_regfile.vrf[16][i*SEW +: SEW]));
        $display("");

        $write("v17:");
        for (i = 0; i < 8; i = i + 1)
            $write(" %0d", $signed(dut.m_v_regfile.vrf[17][i*SEW +: SEW]));
        $display("");
        ...
        $display("Peak architectural VALU throughput = %0d elements/cycle",
                 LANES);
        $display("Peak datapath width               = %0d bits/cycle",
                 VLEN);
        $display("The measured result above is from simulation,");
        $display("while LUT/FF/gate count and Fmax require synthesis.");
    ...
    // ------------------------------------------------------------------------
    initial begin
        $dumpfile("wave_vector_perf.vcd");
        $dumpvars(0, tb_vector_pipeline_perf);
    end
    ...
    initial begin
        #15000;
        $display("[ERROR] Performance TB watchdog timeout.");
        $finish;
    end
